...
app.post('/api/customer-order', (req, res) => {
  ...
  // Si ya hay una cuenta abierta sin mesero para esta mesa, se le agrega ahí; si no, se crea una nueva.
  let tab = state.tabs.find((t) => t.status === 'open' && t.waiter === 'Sin asignar' && t.table === table.name);
  if (!tab) {
    tab = {
