...
function defaultPrices() {
  return {
    ...
    "Corona Extra (cubeta)": { simple: 480 },
    "Corona Light (cubeta)": { simple: 480 },
    "Victoria (cubeta)": { simple: 480 },
    "Modelo Especial/Negra (cubeta)": { simple: 480 },
    "Bud Light (cubeta)": { simple: 480 },
    "Pacífico (cubeta)": { simple: 480 },
    "Michelob Ultra (cubeta)": { simple: 480 },
    "Budweiser (cubeta)": { simple: 480 },
    "Clamato": { simple: 90 },
    "Michelada": { simple: 60 },
    "Chelada": { simple: 40 },
...
function defaultMenu() {
  return [
    { cat: "Cervezas", simple: true, items: [
      "Corona Extra","Corona Light","Victoria","Modelo Especial/Negra","Bud Light","Pacífico","Michelob Ultra","Budweiser",
      "Corona Extra (cubeta)","Corona Light (cubeta)","Victoria (cubeta)","Modelo Especial/Negra (cubeta)","Bud Light (cubeta)","Pacífico (cubeta)","Michelob Ultra (cubeta)","Budweiser (cubeta)"
    ]},
    { cat: "Cheladas y Micheladas", simple: true, items: [
      "Clamato","Michelada","Chelada"
    ]},
    { cat: "Whiskey", formats: true, items: [
      { n: "Buchanans 12", f: ["copeo","litro","botella"] },
      { n: "Buchanans Master", f: ["copeo","litro","botella"] },
      { n: "Buchanans 18", f: ["copeo","litro","botella"] },
      { n: "Buchanans Piña", f: ["copeo","litro"] },
      { n: "Etiqueta Negra", f: ["copeo","litro","botella"] },
      { n: "Double Black", f: ["copeo","litro"] },
      { n: "Jack Daniels", f: ["copeo","litro"] },
      { n: "Chivas Regal", f: ["copeo","litro"] },
      { n: "Macallan 12", f: ["copeo","litro"] },
    ]},
    { cat: "Coctelería Tradicional", formats: true, items: [
      { n: "Margarita", f: ["copeo","litro"] },
      { n: "Mezcalita", f: ["copeo","litro"] },
      { n: "Paloma", f: ["copeo","litro"] },
      { n: "Cuba Libre", f: ["copeo","litro"] },
      { n: "Vampiro", f: ["copeo","litro"] },
      { n: "Mojito", f: ["copeo","litro"] },
      { n: "Perro Salado", f: ["copeo","litro"] },
      { n: "Gin Tonic", f: ["copeo","litro"] },
      { n: "Clericot", f: ["copeo","litro"] },
      { n: "Piña Colada", f: ["copeo","litro"] },
    ]},
    { cat: "Coctelería de la Casa", formats: true, items: [
      { n: "Jagger 43", f: ["copeo","litro"] },
      { n: "Gallo Special", f: ["copeo","litro"] },
      { n: "Gallo Giro", f: ["copeo","litro"] },
      { n: "Gallo de 1/2", f: ["copeo","litro"] },
      { n: "Gallo de Pulgada", f: ["copeo","litro"] },
      { n: "Hot Jimmy", f: ["copeo","litro"] },
    ]},
    { cat: "Sin Alcohol", items: [
      { n: "Piña Colada Virgen", f: ["copeo","litro"] },
      { n: "Limonada Frappé", f: ["copeo","litro"] },
      { n: "Limonada Natural", f: ["copeo","litro"] },
      { n: "Limonada Mineral", f: ["copeo","litro"] },
      { n: "Clericot Virgen", f: ["copeo","litro"] },
      { n: "Refresco", f: [] },
      { n: "Agua", f: [] },
    ], mixed: true },
    { cat: "Shots", simple: true, items: [
      "Perla Negra","Bufanda","Gallito","Don Julio","Maestro Dobel","400 Conejos","Baby Mango","Bandera","Hot Danny"
    ]},
    { cat: "Ron y Vodka", formats: true, items: [
      { n: "Bacardi", f: ["copeo","litro","botella"] },
      { n: "Bacardi Mango", f: ["copeo","litro","botella"] },
      { n: "Smirnoff Tamarindo", f: ["copeo","litro","botella"] },
      { n: "Capitán Morgan", f: ["copeo","litro","botella"] },
      { n: "Absolut Vodka", f: ["copeo","litro","botella"] },
    ]},
    { cat: "Tequila y Mezcal", formats: true, items: [
      { n: "Don Julio 70", f: ["copeo","litro","botella"] },
      { n: "Maestro Dobel", f: ["copeo","litro","botella"] },
      { n: "1800 Cristalino", f: ["copeo","litro","botella"] },
      { n: "Jose Cuervo Esp", f: ["copeo","litro"] },
      { n: "400 Conejos", f: ["copeo","litro"] },
    ]},
  ];
}
...
function defaultIngredients() {
  ...
  const list = [
    ...
    // Cervezas embotelladas y bebidas simples del menú (se venden tal cual, sin receta compuesta)
    pieza('Corona Extra'), pieza('Corona Light'), pieza('Victoria'), pieza('Modelo Especial/Negra'),
    pieza('Bud Light'), pieza('Pacífico'), pieza('Michelob Ultra'), pieza('Budweiser'),
    pieza('Agua'), pieza('Refresco'),
  ];
  const result = {};
...
function defaultState() {
  return {
    waiters: { names: ['Laura', 'Diego', 'Ana', 'Pepe'], claimedBy: {}, onShiftToday: [] },
    tabs: [],
    ...
    dayPromos: [], // [{ id, label, days: [0-6, 0=domingo], price, products: [{name, format}] }] — precios especiales por día
    nextTicketNumber: 1, // consecutivo simple para poder rastrear un ticket impreso en el sistema
    menu: defaultMenu(), // el menú ahora vive aquí (editable desde el panel), no fijo en el código
  ...
  if (fs.existsSync(DATA_FILE)) {
    try {
      ...
      if (parsed && parsed.waiters && Array.isArray(parsed.tabs)) {
        ...
        if (!Array.isArray(parsed.dayPromos)) parsed.dayPromos = [];
        if (typeof parsed.nextTicketNumber !== 'number') parsed.nextTicketNumber = 1;
        if (!Array.isArray(parsed.menu)) parsed.menu = defaultMenu();
        if (!parsed.waiters.onShiftToday) parsed.waiters.onShiftToday = [];
        ...
        // Autogenera recetas simples para cervezas/agua/refresco (1 pieza de sí mismas) y
        // sus cubetas (8 piezas de la cerveza correspondiente) — solo si el usuario no les
        // ha puesto ya su propia receta a mano, para no pisar nada capturado.
        const findIngredientIdByName = (name) => Object.keys(parsed.ingredients).find((id) => parsed.ingredients[id].name === name);
        ['Corona Extra', 'Corona Light', 'Victoria', 'Modelo Especial/Negra', 'Bud Light', 'Pacífico', 'Michelob Ultra', 'Budweiser'].forEach((beerName) => {
          const ingId = findIngredientIdByName(beerName);
          if (!ingId) return;
          const simpleKey = beerName + '|simple';
          if (!parsed.recipes[simpleKey]) parsed.recipes[simpleKey] = [{ ingredientId: ingId, qty: 1 }];
          const cubetaKey = beerName + ' (cubeta)|simple';
          if (!parsed.recipes[cubetaKey]) parsed.recipes[cubetaKey] = [{ ingredientId: ingId, qty: 8 }];
        });
        ['Agua', 'Refresco'].forEach((name) => {
          const ingId = findIngredientIdByName(name);
          if (!ingId) return;
          const key = name + '|simple';
          if (!parsed.recipes[key]) parsed.recipes[key] = [{ ingredientId: ingId, qty: 1 }];
        });
...
// Si hay una promoción activa para HOY que incluya este producto/formato, usa ese precio
// en vez del normal — así "de lunes a jueves la cerveza cuesta 35" se aplica solo.
function getEffectivePrice(name, format) {
  const today = new Date().getDay(); // 0=domingo … 6=sábado
  const promo = state.dayPromos.find((p) =>
    p.days.includes(today) &&
    p.products.some((prod) => prod.name === name && (prod.format || null) === (format || null))
  );
  if (promo) return promo.price;
  return getPrice(name, format);
}

function assignTicketNumber() {
  const n = state.nextTicketNumber;
...
app.post('/api/customer-order', (req, res) => {
  ...
  items.forEach((it) => {
    tab.items.push({
      ...
      note: it.note || '',
      category: null,
      price: getEffectivePrice(it.name, it.format),
      addedAt: now,
    });
  });
...
app.post('/api/shift/close', (req, res) => {
  if (state.shift) {
    state.shiftHistory.push({
      ...
      waitersOnShift: state.waiters.onShiftToday.slice(),
    });
  }
  state.waiters.claimedBy = {};
  state.waiters.onShiftToday = [];
  state.shift = null;
  persistAndBroadcast();
  res.json({ ok: true, state });
});

app.post('/api/waiters/set-onshift', (req, res) => {
  const { names } = req.body || {};
  if (!Array.isArray(names)) return res.status(400).json({ error: 'lista inválida' });
  state.waiters.onShiftToday = names.filter((n) => state.waiters.names.includes(n));
  persistAndBroadcast();
  res.json({ ok: true, state });
});
...
app.post('/api/waiters/remove', (req, res) => {
  ...
  delete state.waiters.claimedBy[name];
  state.waiters.onShiftToday = state.waiters.onShiftToday.filter((n) => n !== name);
  persistAndBroadcast();
  res.json({ ok: true, state });
});
...
app.post('/api/tabs/:id/checkout', (req, res) => {
  const tab = state.tabs.find((t) => t.id === req.params.id);
  if (!tab) return res.status(404).json({ error: 'cuenta no encontrada' });
  const { payments, cardTip, couponCode, changeGiven } = req.body || {};
  ...
  tab.changeGiven = Number(changeGiven) || 0;
  tab.status = 'closed';
...
app.post('/api/day-promos/add', (req, res) => {
  const { label, days, price, products } = req.body || {};
  ...
  if (isNaN(numPrice) || numPrice < 0) return res.status(400).json({ error: 'precio inválido' });
  if (!Array.isArray(products) || products.length === 0) return res.status(400).json({ error: 'agrega al menos un producto' });
  const id = 'promo_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6);
  state.dayPromos.push({
    id,
    label: label ? String(label).trim() : 'Promoción',
    days,
    price: numPrice,
    products: products.map((p) => ({ name: String(p.name), format: p.format || null })),
  });
  persistAndBroadcast();
  res.json({ ok: true, id, state });
});

app.post('/api/day-promos/:id/remove', (req, res) => {
  state.dayPromos = state.dayPromos.filter((p) => p.id !== req.params.id);
  persistAndBroadcast();
  res.json({ ok: true, state });
});

app.post('/api/menu/add-category', (req, res) => {
  const { name, hasFormats } = req.body || {};
  const clean = (name || '').trim();
  if (!clean) return res.status(400).json({ error: 'nombre requerido' });
  if (state.menu.some((c) => c.cat === clean)) return res.status(400).json({ error: 'ya existe una categoría con ese nombre' });
  state.menu.push({ cat: clean, simple: !hasFormats, items: [] });
  persistAndBroadcast();
  res.json({ ok: true, state });
});

app.post('/api/menu/remove-category', (req, res) => {
  const { cat } = req.body || {};
  state.menu = state.menu.filter((c) => c.cat !== cat);
  persistAndBroadcast();
  res.json({ ok: true, state });
});

app.post('/api/menu/add-product', (req, res) => {
  const { cat, name, formats } = req.body || {};
  const category = state.menu.find((c) => c.cat === cat);
  if (!category) return res.status(400).json({ error: 'categoría no encontrada' });
  const clean = (name || '').trim();
  if (!clean) return res.status(400).json({ error: 'nombre requerido' });
  const cleanFormats = Array.isArray(formats) ? formats.map((f) => String(f).trim()).filter(Boolean) : [];
  if (category.simple) {
    if (category.items.includes(clean)) return res.status(400).json({ error: 'ya existe ese producto' });
    category.items.push(clean);
  } else {
    if (category.items.some((p) => p.n === clean)) return res.status(400).json({ error: 'ya existe ese producto' });
    category.items.push({ n: clean, f: cleanFormats });
  }
  persistAndBroadcast();
  res.json({ ok: true, state });
});

app.post('/api/menu/rename-product', (req, res) => {
  const { cat, oldName, newName } = req.body || {};
  const category = state.menu.find((c) => c.cat === cat);
  if (!category) return res.status(400).json({ error: 'categoría no encontrada' });
  const clean = (newName || '').trim();
  if (!clean) return res.status(400).json({ error: 'nombre requerido' });
  if (category.simple) {
    const idx = category.items.indexOf(oldName);
    if (idx === -1) return res.status(400).json({ error: 'producto no encontrado' });
    category.items[idx] = clean;
  } else {
    const item = category.items.find((p) => p.n === oldName);
    if (!item) return res.status(400).json({ error: 'producto no encontrado' });
    item.n = clean;
  }
  persistAndBroadcast();
  res.json({ ok: true, state });
...
app.post('/api/menu/remove-product', (req, res) => {
  const { cat, name } = req.body || {};
  const category = state.menu.find((c) => c.cat === cat);
  if (!category) return res.status(400).json({ error: 'categoría no encontrada' });
  if (category.simple) {
    category.items = category.items.filter((n) => n !== name);
  } else {
    category.items = category.items.filter((p) => p.n !== name);
  }
  persistAndBroadcast();
  res.json({ ok: true, state });
});
...
app.post('/api/ingredients/:id/set-stock', (req, res) => {
  const ing = state.ingredients[req.params.id];
  if (!ing) return res.status(404).json({ error: 'ingrediente no encontrado' });
  const amount = Number((req.body || {}).amount);
  if (isNaN(amount) || amount < 0) return res.status(400).json({ error: 'cantidad inválida' });
